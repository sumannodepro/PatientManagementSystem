...
import { getPrescription } from "../graphql/queries";
import { updatePrescription } from "../graphql/mutations";
export default function PrescriptionUpdateForm(props) {
  const {
    id: idProp,
    prescription: prescriptionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    ...
    setPrescription(cleanValues.prescription);
    setDate(cleanValues.date);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [prescriptionRecord, setPrescriptionRecord] = React.useState(
    prescriptionModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getPrescription.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updatePrescription.replaceAll("__typename", ""),
            variables: {
              input: {
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || prescriptionModelProp)}
