...
import { getPatientTreatment } from "../graphql/queries";
import { updatePatientTreatment } from "../graphql/mutations";
export default function PatientTreatmentUpdateForm(props) {
  const {
    id: idProp,
    patientTreatment: patientTreatmentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = patientTreatmentRecord
      ? { ...initialValues, ...patientTreatmentRecord }
      : initialValues;
    setInvoiceNumber(cleanValues.invoiceNumber);
    setPatientID(cleanValues.patientID);
    setTreatmentID(cleanValues.treatmentID);
    ...
    setModeOfPayment(cleanValues.modeOfPayment);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [patientTreatmentRecord, setPatientTreatmentRecord] = React.useState(
    patientTreatmentModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getPatientTreatment.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPatientTreatment
        : patientTreatmentModelProp;
      setPatientTreatmentRecord(record);
    };
    queryData();
  }, [idProp, patientTreatmentModelProp]);
  React.useEffect(resetStateValues, [patientTreatmentRecord]);
  const validations = {
    invoiceNumber: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updatePatientTreatment.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "PatientTreatmentUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || patientTreatmentModelProp)}
